...
class CmsInput extends CApplicationComponent
{
    ...
    public function stripTags($str, $encode = false)
    {
        ...
        $str = trim(strip_tags($str, join('', $this->allowedTags)));
        if ($encode)
            $str = $this->encode($str);
        return $str;
    }
    ...
    private function getCISecurity()
    {
        ...
        Yii::import('vendor.codeigniter.CI_Security');
    ...
    public function filter($value, $allowed)
    {
    ...
    {
        $this->allowedTags = $allowed['tags'];
        $this->allowedAttr = $allowed['attr'];

        $CI = $this->getCISecurity();
        $CI->setAllowed($allowed);
    }
